...
class StrategyLearner(object):
    ...
    def createIndicators(self):
        ...
        ##keeping/uncommenting only the exact same indicators as manual strategy for comparison
...
if __name__=="__main__":
    ...
    def generatePerformanceChart(df_trades, strategy, manual, benchmark):
        portfolios = np.column_stack((strategy, manual, benchmark))
        portfolios = pd.DataFrame(portfolios, columns =['Random Forest Strategy', 'Manual Strategy', 'Benchmark'], index = strategy.index)
        ...
        ax.plot(portfolios['Manual Strategy'], color = 'c')
        ax.plot(portfolios['Benchmark'], color = 'b')

        ax.set_xlabel("Date", fontsize =12)
        ax.set_ylabel("Portfolio Value")
        ax.set_title("Stock Portfolio Value Over Time")
        ax.legend(loc='best')
    ...
    def run_strategy(start_date, end_date):
        ...
        for sym in symbols:
            ...
            learner.addEvidence(symbol = sym, sd=dt.datetime(2008,1,1), ed=dt.datetime(2009,12,31), sv = 100000)
            df_trades = learner.testPolicy(symbol = sym, sd=start_date, ed=end_date, sv = 100000)        
            df_orders = learner.createOrdersDataFrame(df_trades)

            
            ms = manual.manualStrategy(symbol = sym, sd=start_date, ed=end_date, sv = 100000)
            ...
            portvals_manual = msim.compute_portvals(orders_file = df_orders_ms, start_val = 100000, commission=0, impact=0.005, end_date = end_date, start_date = start_date)
            ...
            generatePerformanceChart(df_trades, portvals_strategy, portvals_manual, portvals_benchmark)
