...
if __name__=="__main__":
    ...
    def run_strategy(start_date, end_date):
        ...
        impact_list = [0.0001, 0.0005, 0.001, 0.005, 0.01, 0.02, 0.05]
        for i in impact_list:
            ...
            if i == impact_list[0]:
                impact_portvals = pd.DataFrame(index = df_trades.index)

            portvals_strategy = msim.compute_portvals(orders_file = df_orders, start_val = 100000, commission=0, impact= i, end_date = end_date, start_date = start_date)
            impact_portvals[i] = portvals_strategy

            print ''
            print 'Statistics for Random Forest Strategy with Impact value: ' + str(i)
            msim.computeStatistics(portvals_strategy)

        #print impact_portvals
        import matplotlib.pyplot as plt

        ax = impact_portvals.plot(title='Portfolio Performance with different values of Impact', fontsize=10)
        ax.set_xlabel('Date')
        ax.set_ylabel('Portfolio Value')
        plt.show()
